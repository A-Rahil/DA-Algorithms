# Design and Analysis of Algorithms

This repository contains implementations of fundamental algorithms covered in the Design and Analysis of Algorithms course, crreated as a resource for students.

## Overview

**Language:** Python

The implementations focus on the logic and practical application of standard algorithms across Sorting, Graph Theory, Greedy Methods, and Dynamic Programming.

## Contents

### 1. Sorting
* [Insertion Sort](Sorting/insertionSort.py)
* [Selection Sort](Sorting/selectionSort.py)
* [Merge Sort](Sorting/mergeSort.py)
* [Quick Sort](Sorting/quickSort.py)
* [Heap Sort](Sorting/heaps.py)
* [Counting Sort](Sorting/countingSort.py)
* [Radix Sort](Sorting/radixSort.py)

### 2. Greedy Algorithms & Graph Traversals
* [Fractional Knapsack](GA/fracKnapsack.py)
* [Job Scheduling](GA/intervalScheduling.py)
* [Breadth-First Search (BFS)](GA/Graphs/BFS.py)
* [Depth-First Search (DFS)](GA/Graphs/DFS.py)
* [Prim's Algorithm](GA/Graphs/prims.py)
* [Kruskal's Algorithm](GA/Graphs/kruscal.py)
* [Dijkstra's Algorithm](GA/Graphs/DJA.py)
* [Bellman-Ford Algorithm](GA/Graphs/bellmanFord.py)

### 3. Dynamic Programming
* [Fibonacci Sequence](DP/fib.py)
* [Longest Common Subsequence](DP/LCS.py)
* [Weighted Job Scheduling](DP/wJobSc.py)
* [0/1 Knapsack](DP/01Knapsack.py)

## How to Run

1. Clone the repository:
   ```bash
   git clone [https://github.com/A-Rahil/DA-Algorithms.git](https://github.com/A-Rahil/DA-Algorithms.git)
